...
const StatCard = ({ title, value, unit, subtitle, icon: Icon, color = 'green', trend }) => {
  ...
  return (
    <div className="p-5 rounded-[16px] bg-white dark:bg-slate-800 border border-line dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden">
      <div className="flex items-center justify-between mb-3">
        <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider font-mono">{title}</span>
        {Icon && (
      ...
      <div className="flex items-baseline space-x-1.5 mb-1">
        <span className="font-mono text-2xl font-bold text-navy-900 dark:text-slate-200 tracking-tight">{value}</span>
